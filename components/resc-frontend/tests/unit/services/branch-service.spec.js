...
describe('function getFindingsCountByStatusForBranch', () => {
  let branchId = 1;
  let mocked_branch_findings_metadata = {
    data: {
      branch_id: 'b1',
      branch_name: 'master',
  ...
  describe('when API call is successful for getFindingsCountByStatusForBranch', () => {
    it('should return branch with findings metadata', async () => {
      axios.get.mockResolvedValueOnce(mocked_branch_findings_metadata);

      const response = await BranchService.getFindingsCountByStatusForBranch(branchId);
      expect(response).toEqual(mocked_branch_findings_metadata);
