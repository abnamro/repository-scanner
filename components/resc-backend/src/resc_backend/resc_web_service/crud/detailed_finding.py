...
def get_detailed_findings(db_connection: Session, findings_filter: FindingsFilter, skip: int = 0,
    ...
    latest_audit_ids_subquery = db_connection.query(func.max(model.audit.DBaudit.id_)) \
        .join(model.DBfindingAudit,
              model.audit.DBaudit.id_ == model.finding_audit.DBfindingAudit.audit_id) \
        .group_by(model.finding_audit.DBfindingAudit.finding_id).subquery()
    ...
    query = db_connection.query(
        ...
        .join(model.DBfindingAudit,
              model.finding_audit.DBfindingAudit.finding_id == model.finding.DBfinding.id_) \
        .join(model.DBaudit,
              model.audit.DBaudit.id_ == model.finding_audit.DBfindingAudit.audit_id) \
        .filter(model.audit.DBaudit.id_.in_(latest_audit_ids_subquery))
...
def get_detailed_finding(db_connection: Session, finding_id: int) -> detailed_finding_schema.DetailedFindingRead:
    ...
    # subquery to select latest audit ids of findings
    latest_audit_id_subquery = db_connection.query(func.max(model.audit.DBaudit.id_)) \
        .join(model.DBfindingAudit,
              model.audit.DBaudit.id_ == model.finding_audit.DBfindingAudit.audit_id
              and model.audit.DBaudit.finding_id == finding_id) \
        .group_by(model.finding_audit.DBfindingAudit.finding_id).subquery()
    ...
    finding = db_connection.query(
        ...
        .join(model.DBaudit,
              model.audit.DBaudit.id_ == model.finding_audit.DBfindingAudit.audit_id) \
        .filter(model.audit.DBaudit.id_.in_(latest_audit_id_subquery)) \
