...
def prompt_questions():
    try:
        operating_system_input = questionary.select(
        ...
        if operating_system_input:
            operating_system = get_operating_system(operating_system_input=operating_system_input)

        if operating_system:
            db_storage_path, rabbitmq_storage_path = create_storage_for_db_and_rabbitmq(operating_system=operating_system)
        ...
        if (operating_system and db_password and db_storage_path and rabbitmq_storage_path and github_username and github_token):
